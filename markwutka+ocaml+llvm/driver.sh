...
if [ "$MODE" = "--compile" ]; then
  mv $ASSEMBLED __asmtemp
  if [ $UNAME = "Darwin" ]; then
    if [ $ARCH = "arm64" ]; then
      echo 'target triple = "arm64-apple-macosx26.0.0"' > $ASSEMBLED
    fi
    if [ $ARCH = "x86_64" ]; then
      echo 'target triple = "x86_64-apple-macosx26.0.0"' > $ASSEMBLED
    fi
  fi
  if [ $UNAME = "Linux" ]; then
    if [ $ARCH = "arm64" ]; then
      echo 'target triple = "arm64-pc-linux-gnu"' > $ASSEMBLED
    fi
    if [ $ARCH = "x86_64" ]; then
      echo 'target triple = "x86_64-pc-linux-gnu"' > $ASSEMBLED
    fi
  fi
  cat __asmtemp >> $ASSEMBLED
  rm __asmtemp
  clang $ASSEMBLED -o $BASE
  ERR=$?
  rm $ASSEMBLED
  exit $ERR
fi
